...
const getCategory = async (req, res) => {
  ...
  if (!category) {
    return res.status(204).json({ "message": `No category matches the parameter ${req.params.slug}. `});
  }
  res.json(category);
};
...
const updateCategory = [ 
  ...
  async (req, res) => {
    ...
    const category = await Category.findOne({ slug: req.params.slug }).exec();
    if (!category) {
      return res.status(404).json({ "message": `Your reference ${req.body.slug} does not match our category records.` });
    }
    if (req.body?.name) category.name = req.body.name;
    category.slug = (req.body.name).toLowerCase().replaceAll(' ', '-');
